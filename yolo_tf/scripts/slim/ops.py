...
def softmax(logits, scope=None):
  ...
  with tf.variable_op_scope([logits], scope, 'Softmax'):
    ...
    if len(dims) < 2:
      raise ValueError('rank of shape must be at least 2 not: %d' % (len(dims)))
    num_logits = dims[-1].value
    if num_logits is None:
      raise ValueError('last dimension shape must be known but is None')

    logits_2d = array_ops.reshape(logits, [-1, num_logits])
    predictions = tf.nn.softmax(logits_2d)
    predictions = tf.reshape(predictions, tf.shape(logits))
...
def max_pool2d(inputs,
               ...
               scope=None):
  assert outputs_collections is None, 'outputs_colections is not supported.'
  if data_format not in (DATA_FORMAT_NCHW, DATA_FORMAT_NHWC):
    raise ValueError('data_format has to be either NCHW or NHWC.')
  with tf.variable_op_scope([inputs], scope, 'MaxPool2D'):
    ...
    if stride==[1,2,2,1]:
      ...
      out = tf.reshape(inputs, [-1, int(shape[1]/2), 2, shape[2], shape[3]])
      out = tf.transpose(out, [0,1,3,2,4])
      out = tf.reshape(out, [-1, int(shape[1]/2), int(shape[2]/2), 4, shape[3]])
      return tf.reduce_max(out, 3)
    else:
      out = tf.pad(inputs, [[0,0],[0,2],[0,2],[0,0]])
      shape = out.get_shape().as_list()
      shape[1] -= 1
      shape[2] -= 1
      out11 = tf.reshape(out[:,:shape[1],:shape[2],:], [-1, int(shape[1]/2), 2, shape[2], shape[3]])
      out11 = tf.transpose(out11, [0,1,3,2,4])
      out11 = tf.reshape(out11, [-1, int(shape[1]/2), int(shape[2]/2), 4, shape[3]])
      out11 = tf.reduce_max(out11, 3)
      out12 = tf.reshape(out[:,:shape[1],1:,:], [-1, int(shape[1]/2), 2, shape[2], shape[3]])
      out12 = tf.transpose(out12, [0,1,3,2,4])
      out12 = tf.reshape(out12, [-1, int(shape[1]/2), int(shape[2]/2), 4, shape[3]])
      out12 = tf.reduce_max(out12, 3)
      out21 = tf.reshape(out[:,1:,:shape[2],:], [-1, int(shape[1]/2), 2, shape[2], shape[3]])
      out21 = tf.transpose(out21, [0,1,3,2,4])
      out21 = tf.reshape(out21, [-1, int(shape[1]/2), int(shape[2]/2), 4, shape[3]])
      out21 = tf.reduce_max(out21, 3)
      out22 = tf.reshape(out[:,1:,1:,:], [-1, int(shape[1]/2), 2, shape[2], shape[3]])
      out22 = tf.transpose(out22, [0,1,3,2,4])
      out22 = tf.reshape(out22, [-1, int(shape[1]/2), int(shape[2]/2), 4, shape[3]])
      out22 = tf.reduce_max(out22, 3)
      out = tf.stack([out11,out12,out21,out22]) #[4,N,H/2,W/2,C]
      out = tf.transpose(out, [1,2,3,0,4]) #[N,H/2,W/2,4,C]
      out = tf.reshape(out, [-1,int(shape[1]/2), int(shape[2]/2), 2, 2*shape[3]])#[N,H/2,W/2,2,2*C]
      out = tf.transpose(out, [0,1,3,2,4]) #[N,H/2,2,W/2,2*C]
      out = tf.reshape(out, [-1,shape[1],shape[2],shape[3]])
      return out[:,:shape[1]-1,:shape[2]-1,:]

    #return tf.nn.max_pool(inputs, kernel_size, stride, padding,
...
def avg_pool2d(inputs,
  ...
  if data_format not in (DATA_FORMAT_NCHW, DATA_FORMAT_NHWC):
    raise ValueError('data_format has to be either NCHW or NHWC.')
  with tf.variable_op_scope([inputs], scope, 'AvgPool2D'):
    inputs = tf.convert_to_tensor(inputs)
    if isinstance(kernel_size, int):
      kernel_size = [kernel_size, kernel_size]
    kernel_size = [1]+kernel_size+[1]
...
def repeat(inputs, repetitions, layer, *args, **kwargs):
  ...
  with tf.variable_op_scope([inputs], scope, 'Repeat'):
    inputs = tf.convert_to_tensor(inputs)
    if scope is None:
