...
class Yolo9000Train:
    def __init__(self, ckpt_file, bg_dir, fg_dir, device='/cpu:0'):
    ...
    def _find_files(self, bg_dir, fg_dir):
        ...
        for f in os.listdir(fg_dir):
            ...
            img_key_name = fg_dir + f
            if name not in self.class_names:
                self.class_names.append(name)
                self.fg_files[name] = []
            self.fg_files[name].append(os.path.join(fg_dir, f))
            self.img_files[img_key_name] = cv2.imread(os.path.join(fg_dir, f), -1)
    ...
    def train(self, learn_from_yolo9000=False, show_detection=False):
        ...
        batch = self.create_image_batch(size=5, num_objs=3)
        if batch is None:
            return np.nan
        img, bbox, cls = batch
