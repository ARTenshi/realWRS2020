...
class RobotInfo(object):
    ...
    def make_obj_list(self, task):
        ...
        # which room
        if task == "tidy up":
            PATH_TO_XML += "objects_tidy_up.xml"
        elif task == "go get it":
            PATH_TO_XML += "objects_go_get_it.xml"
        elif task == "clean up":
            PATH_TO_XML += "objects_clean_up.xml"
        elif task == "storing groceries":
            PATH_TO_XML += "objects_storing_groceries.xml"
        elif task == "clean table":
            PATH_TO_XML += "objects_clean_table.xml"
        else:
            PATH_TO_XML += "objects.xml"
    ...
    def make_location_list(self, task):
        ...
        if task == "tidy up":
            ...
            PATH_TO_XML += "locations_go_get_it.xml"
            self.start_area = "living room"
        elif task == "clean up":
            PATH_TO_XML += "locations_clean_up.xml"
            self.start_area = "living room"
        elif task == "storing groceries":
            PATH_TO_XML += "locations_storing_groceries.xml"
            self.start_area = "kitchen"
        elif task == "clean table":
            PATH_TO_XML += "locations_clean_table.xml"
            self.start_area = "kitchen"
        else:
            PATH_TO_XML += "locations.xml"
            self.start_area = "living room"
	...
	idx = 0
	counter = 0
	while counter < len(obsrv_pt_list):
		next = False
		while next == False:
			for i in range(len(obsrv_pt_list)):
				if ('-'+str(idx)+' ') in obsrv_pt_list[i].name:
					self.obsrv_pt_list.append(obsrv_pt_list[i])
					next = True
					i = len(obsrv_pt_list)
					break
			idx+=1
