...
void callback_enable(const std_msgs::Bool::ConstPtr& msg)
{
	...
	pub_cmd_vel   = n->advertise<geometry_msgs::Twist>("/hri/human_speed_calculator/human_speed", 1);
...
int main(int argc, char** argv)
{
    ...
    std::cout << "INITIALIZING HUMAN SPEED CALCULATOR BY EDD-2..." << std::endl;
    ros::init(argc, argv, "human_speed_calculator");
    n = new ros::NodeHandle();

    if(ros::param::has("~distance"))
      ros::param::get("~distance", distance2person);

    ros::Subscriber sub_enable = n->subscribe("/hri/human_speed_calculator/start_calculator", 1, callback_enable);
    ros::Rate loop(20);
