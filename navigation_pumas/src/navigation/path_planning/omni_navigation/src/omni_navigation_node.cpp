				      ...
				      float cruise_speed, bool backwards)
{
    ...
    error = sqrt((robot_x - goal_x)*(robot_x - goal_x) + (robot_y - goal_y)*(robot_y - goal_y));
    error_x = goal_x - robot_x;
    error_y = goal_y - robot_y;

    //Error calculation
    // float angle_error = atan2(goal_y - robot_y, robot_x - goal_x) - robot_t;
    if(robot_t >   M_PI) robot_t -= 2*M_PI;
    if(robot_t <= -M_PI) robot_t += 2*M_PI;
    
    geometry_msgs::Twist result;
     vx  = cruise_speed * exp(-(error_y * error_y) / alpha) *
      error_x * beta;
     vy = cruise_speed * exp(-(error_x * error_x) / alpha) *
      error_y * beta;

    //low-pass filter
    vx  = v_x_ant*(1-time_const) + lp_const * time_const * vx;
    vy  = v_y_ant*(1-time_const) + lp_const * time_const * vy;

    //Angle projections
    if(robot_t > 0.0 && robot_t < 1.5707)
    {
      result.linear.x = vx * cos(-robot_t) - vy * sin(-robot_t);
      result.linear.y = vy * cos(-robot_t) + vx * sin(-robot_t);
    }
    else if(robot_t < 0.0 && robot_t > -1.5707)
    {
      result.linear.x = vx * cos(robot_t) + vy * sin(robot_t);
      result.linear.y = vy * cos(robot_t) - vx * sin(robot_t);
    }
    ...
    v_x_ant = result.linear.x;
    v_y_ant = result.linear.y;

    std::cout << "   ------------------------------------------  " << std::endl;
    std::cout << "Theta: " << atan2(goal_y - robot_y, robot_x - goal_x) << std::endl;
    std::cout << "Robot_angle: " << robot_t << std::endl;
    //std::cout << "angle_error: " << angle_error << std::endl;
    std::cout << "Error_x: " << goal_x - robot_x << "  - Speed_x: " << result.linear.x << std::endl;
    std::cout << "Error_y: " << goal_y - robot_y << "  - Speed_y: " << result.linear.y << std::endl;
    
    result.angular.z = 0.0;

    // geometry_msgs::Twist dump;
    // return dump;
    return result;	       	
}
...
void get_goal_position_wrt_robot(float goal_distance, float goal_angle, tf::TransformListener& tf_listener,
				float& goal_x, float& goal_y, float& goal_t, bool lateral)
{
    tf::StampedTransform transform;
    tf_listener.lookupTransform("base_link", "odom", ros::Time(0), transform);

    tf::Vector3 point(goal_x, goal_y, 0.0);
    tf::Vector3 goal_pos = transform * point;
                             ...
                             int& next_pose_idx, tf::TransformListener& tf_listener)
{
    ...
    float hyp_error = 1000000000;
    float hyp_goal_x;
    float hyp_goal_y;
    float temp_error;
    int   hyp_next_pose_idx;
    for(int i = 0; i < goal_path.poses.size(); i++){
      hyp_goal_x = goal_path.poses[i].pose.position.x;
      hyp_goal_y = goal_path.poses[i].pose.position.y;
      temp_error  = sqrt( (hyp_goal_x - robot_x)*(hyp_goal_x - robot_x)
			  + (hyp_goal_y - robot_y)*(hyp_goal_y - robot_y));
      if(temp_error < hyp_error){
	hyp_error = temp_error;
	hyp_next_pose_idx = i;
	goal_x = hyp_goal_x;
	goal_y = hyp_goal_y;
      }
	...
	//}while(error < 0.25 && ++next_pose_idx < goal_path.poses.size());
      }while(error < 0.35 && ++next_pose_idx < goal_path.poses.size());
    }
    else
    {
      error = hyp_error;
      next_pose_idx = hyp_next_pose_idx;
...
int main(int argc, char** argv)
{
    ...
    // ros::Publisher  pub_cmd_vel          = n.advertise<geometry_msgs::Twist>("/hardware/mobile_base/cmd_vel_path", 1);
