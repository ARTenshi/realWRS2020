...
//<+front, +up, +left>
double dx_cam = 0.022, dy_cam = 0.215, dz_cam = 0.06;
...
cv::Mat changeOrientation(pcl::PointCloud<pcl::PointXYZRGB>::Ptr pclCloud)
{
  ...
  double diff_z = dy_cam * sin( fabs(angle_pitch) );
  double diff_y = dy_cam * (1 - cos(fabs(angle_pitch)) );
  ...
  for (int h = 0; h < pointCloud.rows; h++)
  	for (int w = 0; w < pointCloud.cols; w++)
	{
		...
		if ( !std::isnan(pclCloud->at(w, h).z) )
		{
			...
			//Position correction
			p.x = p.x + dx_cam;
			p.y = dy_torso + dy_cam - diff_y - p.y + dy_offset;
			p.z = p.z - dz_cam + diff_z;
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < argc; i++)
  {
	  ...
	  if ( strcmp( argv[i], "-dy_offset") == 0 )
		  dy_offset = atof(argv[++i]);

	  if ( strcmp( argv[i], "-dy_torso") == 0 )
		  dy_torso = atof(argv[++i]);
