...
bool srvCallback(erasers_nav_msgs::GetClosestPoint::Request &req, erasers_nav_msgs::GetClosestPoint::Response &res)
{
  ...
  metric_point.at<float>(0) = metric_point.at<float>(0) - float(dx_cam) + float(diff_x);
  metric_point.at<float>(1) = metric_point.at<float>(1) - float(dy_cam);
  metric_point.at<float>(2) = dz_torso + dz_cam  - float(diff_z) - metric_point.at<float>(2) + dz_offset;
