...
//Head position wrt the robot in robot coordinates
//<+front, +left, +up>
double dx_cam = 0.06, dy_cam = 0.022, dz_cam = 0.215;
...
bool srvCallback(erasers_nav_msgs::GetLinePoint::Request &req, erasers_nav_msgs::GetLinePoint::Response &res)
{
  ...
  double diff_x = dz_cam * sin( fabs(angle_pitch) );
  double diff_z = dz_cam * (1 - cos(fabs(angle_pitch)) );
  ...
  double miny = dz_torso + dz_cam + dz_offset - diff_z - povParams.areaLimits.maxY;
  double maxy = dz_torso + dz_cam + dz_offset - diff_z - povParams.areaLimits.minY;
  ...
  metric_start.at<float>(0) = metric_start.at<float>(0) - float(dx_cam) + float(diff_x);
  metric_start.at<float>(1) = metric_start.at<float>(1) - float(dy_cam);
  metric_start.at<float>(2) = dz_torso + dz_cam  - float(diff_z) - metric_start.at<float>(2) + dz_offset;
  //metric_start.at<float>(2) = float(dz_torso + dz_cam);

  metric_end.push_back(getCloudPoint(line.row(1)));
  metric_end.convertTo(metric_end, CV_32F);

  metric_end.at<float>(0) = metric_end.at<float>(0) - float(dx_cam) + float(diff_x);
  metric_end.at<float>(1) = metric_end.at<float>(1) - float(dy_cam);
  metric_end.at<float>(2) = dz_torso + dz_cam  - float(diff_z) - metric_end.at<float>(2) + dz_offset;
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < argc; i++)
  {
	  ...
	  //camera offset
	  if ( strcmp( argv[i], "-dx_cam") == 0 )
		  dx_cam = atof(argv[++i]);

	  if ( strcmp( argv[i], "-dy_cam") == 0 )
		  dy_cam = atof(argv[++i]);

	  if ( strcmp( argv[i], "-dz_cam") == 0 )
		  dz_cam = atof(argv[++i]);

	  if ( strcmp( argv[i], "-dy_offset") == 0 )
		  dz_offset = atof(argv[++i]);

	  if ( strcmp( argv[i], "-dy_torso") == 0 )
		  dz_torso = atof(argv[++i]);
